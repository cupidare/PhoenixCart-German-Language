<?php
/*
  $Id$

  CE Phoenix, E-Commerce made Easy
  https://phoenixcart.org

  Copyright (c) 2021 Phoenix Cart

  Released under the GNU General Public License
*/

const TEXT_STORE_NAME = 'Name des Shops';
const TEXT_STORE_NAME_PLACEHOLDER = 'Name des Shops';
const TEXT_STORE_NAME_EXPLANATION = '<small class="form-text text-muted">Der Name des Shops, der den Kunden angezeigt wird.</small>';
const TEXT_OWNER_NAME = 'Name des Shop-Besitzers';
const TEXT_OWNER_NAME_PLACEHOLDER = 'Ihr Name';
const TEXT_OWNER_NAME_EXPLANATION = '<small class="form-text text-muted">Der Name des Shopbesitzers, der den Kunden angezeigt wird.</small>';
const TEXT_OWNER_EMAIL = 'Email-Adresse des Shop-Besitzers';
const TEXT_OWNER_EMAIL_PLACEHOLDER = 'you@yours.com';
const TEXT_OWNER_EMAIL_EXPLANATION = '<small class="form-text text-muted">Die Email-Adresse des Shopbesitzers, der den Kunden angezeigt wird.</small>';
const TEXT_ADMIN_USERNAME = 'Admin Nutzername';
const TEXT_ADMIN_USERNAME_PLACEHOLDER = 'Nutzername';
const TEXT_ADMIN_USERNAME_EXPLANATION = '<small class="form-text text-muted">Der Admin-Nutzername für das Admin-Backend.</small>';
const TEXT_ADMIN_PASSWORD = 'Admin-Passwort';
const TEXT_ADMIN_PASSWORD_EXPLANATION = '<small class="form-text text-muted">Das Passwort für den Admin-Account.</small>';
const TEXT_ADMIN_DIRECTORY = 'Name vom Admin-Verzeichnis';
const TEXT_ADMIN_DIRECTORY_EXPLANATION = '<small class="form-text text-muted">Hierhin werden die Admindateien installiert. Sie sollten es aus Sicherheitsgründen ändern.</small>';
const TEXT_TIME_ZONE = 'Zeitzone';
const TEXT_TIME_ZONE_EXPLANATION = '<small class="form-text text-muted">Die Zeitzone, die für den Shop genutzt werden soll.</small>';
const TEXT_CONTINUE_STEP_4 = 'Weiter zu Schritt 4';
const TEXT_STEP_3 = 'Schritt 3';
const TEXT_STORE_SETTINGS_EXPLANATION = <<<'EOT'
<p>Hier können sie den Name des Onlineshops und die Kontaktinformationen des Besitzers hinterlegen.
Dies kann auch später im Admin-Bereich erfolgen.</p>
<p>Der Nutzername und das Passwort werden benötigt, um sich im Admin-Tool einzuloggen.</p>
EOT;
